...
class Recognition_Pipeline:
    ...
    def recognize(self, img, detector, recognizer):

        #1.Text Detection
        img=cv2.resize(img,(512,512))

        pred_im=detector.predict(np.expand_dims(img,axis=0))
        ...
        index_char = self.__getIndexChar()
        if len(rotateParas) > 0:
            for num in range(len(rotateParas)):
                ...
                img1=tf.keras.preprocessing.image.random_rotation(img1,angle)
                img1=tf.image.crop_to_bounding_box(img1,crop[1],crop[0],crop[3],crop[2]).numpy()
                img1=cv2.resize(img1,(128,64))
                img1=cv2.detailEnhance(img1)
                pred_text=recognizer.predict(np.expand_dims(img1,axis=0))
                arr=tf.keras.backend.ctc_decode(pred_text,np.ones((1),'int8')*64,)
        ...
        for i in range(len(txts)):
            ...
            cv2.putText(img,txts[i],(pts[i][0][0],pts[i][0][1]),cv2.FONT_HERSHEY_SIMPLEX,1, (200,50,0), 2)
